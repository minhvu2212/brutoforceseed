...
semaphore = asyncio.Semaphore(5)  # Giới hạn số lượng yêu cầu đồng thời
...
async def check_solana_balance_and_save(session, seed):
    ...
    try:
        ...
        async with semaphore, session.post(solana_client, json={"method": "getBalance", "params": [str(pubkey)], "id": 1, "jsonrpc": "2.0"}) as response:
...
async def check_evm_balance_and_save(session, seed, blockchain_type):
    ...
    for attempt in range(len(endpoints)):
        ...
        try:
            log_message(f"Checked for {seed}")
            async with semaphore, session.post(endpoint, json={"jsonrpc":"2.0","method":"eth_getBalance","params":[address, "latest"],"id":1}) as response:
                result = await response.json()
