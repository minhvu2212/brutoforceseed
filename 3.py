...
BLOCKCHAIN_TYPES = {
    'ETH': {
        'endpoints': ['https://ethereum-rpc.publicnode.com', 'https://rpc.ankr.com/eth', 'https://rpc.immutable.com', 'https://1rpc.io/eth'],
        'name': 'Ethereum',
        'web3': True
    },
    'ETC': {
        'endpoints': ['https://etc.rivet.link/','https://geth-at.etc-network.info', 'https://etc.etcdesktop.com', 'https://besu-at.etc-network.info'],
        'name': 'Ethereum Classic',
        'web3': True
    },
    'BSC': {
        'endpoints': ['https://bsc-dataseed.binance.org/','https://bsc-dataseed2.defibit.io','https://rpc.ankr.com/bsc', 'https://bsc.nodereal.io/', 'https://bsc-dataseed3.defibit.io', 'https://bsc-dataseed1.defibit.io'],
...
def check_evm_balance_and_save(seed, blockchain_type):
    ...
    for attempt in range(len(endpoints)):
        ...
        try:
            ...
            if 'Too Many Requests' in str(e):
                ...
                asyncio.run(asyncio.sleep(random.uniform(1, 3)))  # Chờ một chút trước khi thử lại
            else:
    ...
    if mnemo.check(mnemonic):
        ...
        for blockchain_type in selected_blockchains:
            if blockchain_type == 'SOL':
                ...
                tasks.append(asyncio.get_running_loop().run_in_executor(executor, check_evm_balance_and_save, mnemonic, blockchain_type))
...
async def brute_force(attempts, selected_blockchains, max_workers):
    ...
    async with aiohttp.ClientSession() as session:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            tasks = [brute_force_task(session, selected_blockchains, executor) for _ in range(attempts)]
            await asyncio.gather(*tasks)
...
max_threads_label.grid(row=7, column=1, pady=5, padx=10, sticky='w')
...
for i, blockchain_type in enumerate(BLOCKCHAIN_TYPES.keys()):
    ...
    chk_btn.grid(row=3 + i // 2, column=i % 2, pady=5, padx=10, sticky='w')

# Nhãn để hiển thị số lượng mnemonics hợp lệ được tìm thấy
valid_count_label = ttk.Label(root, text="Valid Mnemonics Found:", background="#2f2f2f", foreground="#ececec", font=font_style)
valid_count_label.grid(row=8, column=0, pady=10, padx=10, sticky='w')

# Nhãn phụ để hiển thị số đếm
valid_count_number_label = ttk.Label(root, text="0", background="#2f2f2f", foreground="red", font=font_style)
valid_count_number_label.grid(row=8, column=1, pady=10, padx=10, sticky='w')
